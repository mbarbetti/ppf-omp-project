 ...
 * In this script, the parallelization is based on the collapsing of the nested 
 * for loops (lines 94-104) that compute the Mandelbrot set. A "runtime" scheduling 
 * clause is used, then you are free to decide the preferred scheduling strategy 
 * at runtime.
 *
 * Compile with:
 * gcc -std=c99 -Wall -Wpedantic -fopenmp my-mandelbrot-collapse.c -o my-mandelbrot-collapse.out
 *
 * Run with:
 * OMP_NUM_THREADS=4 OMP_SCHEDULE="static,1" ./my-mandelbrot-collapse.out [x_size y_size]
 *
