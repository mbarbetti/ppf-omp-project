...
# This bash script executes ./my-mandelbrot.out in "weak scaling"
# condition, namely expanding the input data-size so that the work 
# is linearly increasing with the number of processors used (P). In
# particular, the program is executed using different P values from
# 1 to the total cores available to the machine, while the picture
# sizes of the Mandelbrot set are expanded accordingly to the 
# formula: new_size = base_size * sqrt(P), where base_size is the
# one used with P = 1. The expected behavior is that the execution 
# time of the program remains constant although one varies P, and
# that the speedup is equal to 1 everywhere.

# Run with:
# source weak-scaling.sh <scheduling_type> <scheduling_chunksize>
