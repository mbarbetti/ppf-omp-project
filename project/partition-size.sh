...
# This bash script executes ./my-mandelbrot.out using different
# scheduling strategies. In particular, fixed the number of threads
# and the scheduling type within {'static', 'dynamic'}, the script 
# tests the program time performance, varying the scheduling
# chunksize from 2 to 256.

# Run with:
# source partition-size.sh <num_threads> <scheduling_type>
...
for chunksize in 1 2 4 8 16 32 64 128 256; do
    ...
    for rep in `seq 20`; do
        EXEC_TIME="$( OMP_NUM_THREADS=$1 OMP_SCHEDULE="$2,$chunksize" ./my-mandelbrot.out $PROB_XSIZE $PROB_YSIZE | sed 's/Elapsed time: //' )"
        echo -n -e "\t${EXEC_TIME}"
    done
    echo ""
done
