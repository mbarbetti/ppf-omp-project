...
if ("static" in filename) or ("dynamic" in filename):
  text = filename[:-4].split("_")[-1]
  text = text.split("c")
...
ax.plot ( processors, t_ideal, color = "black", ls = "--", 
          lw = 0.75, label = "ideal Elapsed time", zorder = 0 )
ax.errorbar ( processors, t_mean, yerr = t_errs, marker = "o", markersize = 2, capsize = 2, 
              elinewidth = 1, lw = 0.75, label = "measured Elapsed time", zorder = 1 )
